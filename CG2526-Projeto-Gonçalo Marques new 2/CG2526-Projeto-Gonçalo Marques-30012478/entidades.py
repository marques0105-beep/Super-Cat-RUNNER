...
# Nota: entidades usam configuracoes.GROUND_Y dinamicamente para evitar cópias obsoletas.
...
class Gato:
    def __init__(self):
        ...
        # usa GROUND_Y atual do módulo configuracoes
        self.Y_POS = configuracoes.GROUND_Y - self.rect.height
...
class Tronco:
    def __init__(self):
        ...
        scale = height / base_h if base_h > 0 else 1.0
        ...
        # Alinha a base do tronco com o GROUND_Y atual do módulo configuracoes.
        # Subtraímos 2 px para compensar sombras/padding; ajusta se necessário.
        self.rect.bottomleft = (WIDTH + 50, configuracoes.GROUND_Y - 2)
...
class Bird:
    def __init__(self):
        ...
        base_height = configuracoes.GROUND_Y - 140
        alt1 = base_height
        alt2 = base_height - 60
        y = random.choice([alt1, alt2])
        self.rect = self.image.get_rect()
        self.rect.topleft = (WIDTH + 50, y)
...
class PowerUp:
    def __init__(self):
        ...
        self.base_y = configuracoes.GROUND_Y - 140
...
class Boss:
    ...
    # ... os métodos update_* e demais métodos mantêm-se idênticos ao teu código original.
    def update(self, player_rect):
        ...
        bottom_limit = configuracoes.GROUND_Y + 40
        if self.rect.y < top_limit:
            self.rect.y = top_limit
        if self.rect.y > bottom_limit:
            self.rect.y = bottom_limit

    # Copia o resto dos métodos (choose_next_state, update_normal, update_smash, ...) do teu ficheiro original.
    ...
    def update_smash(self, player_rect):
        if self.smash_phase == "up":
            ...
            if self.rect.bottom >= configuracoes.GROUND_Y + 8:
                self.rect.bottom = configuracoes.GROUND_Y + 8
                self.state = "returning"
