...
def create_ground_surface(bg_surface, ground_y, height=56):
    # Cria superfície do chão baseada no bg (média de cor)
    try:
...
# -------- Persistência de progresso (unlocked_levels) --------
def load_progress():
    try:
        with open(PROGRESS_FILE, "r") as f:
            return int(json.load(f).get("unlocked_levels", 1))
    except Exception:
        return 1
...
def save_progress(unlocked):
    try:
        with open(PROGRESS_FILE, "w") as f:
            json.dump({"unlocked_levels": int(unlocked)}, f)
    except Exception as e:
...
def draw_progress_map(surface, score, level):
    ...
    cat_w = 18; cat_h = 18
    cat_x = bar_x - cat_w - 12; cat_y = bar_y - cat_h // 2
    ...
    flag_x = bar_x + bar_w + 12; flag_y = bar_y - 20; pole_h = 40
    ...
    marker_x = int(bar_x + progress * bar_w); marker_y = bar_y
