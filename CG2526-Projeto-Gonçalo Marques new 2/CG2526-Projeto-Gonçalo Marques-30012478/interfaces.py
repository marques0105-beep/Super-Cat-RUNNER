...
from funcionalidades import draw_button, load_highscore
import math

# As funções de tela usam screen passado como argumento (sem import circular).
...
def level_select_menu(screen):
    ...
    for i, lvl in enumerate(levels):
        ...
        buttons.append({"label": f"Level {lvl}", "rect": (x,y,btn_w,btn_h), "level": lvl})
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key in (pygame.K_DOWN, pygame.K_s):
                    ...
                    lvl = chosen["level"]
                    if lvl <= configuracoes.unlocked_levels:
        ...
        for i, b in enumerate(buttons):
            lvl = b["level"]
            disabled = lvl > configuracoes.unlocked_levels
...
def menu(screen):
    ...
    while True:
        ...
        for dx, dy in [(-2,0),(2,0),(0,-2),(0,2)]:
            ...
            screen.blit(shadow, (WIDTH//2 - shadow.get_width()//2 + dx, int(HEIGHT*0.12) + dy))
        screen.blit(title, (WIDTH//2 - title.get_width()//2, int(HEIGHT*0.12)))
        for i, b in enumerate(buttons):
            is_selected = (i == selected_idx)
