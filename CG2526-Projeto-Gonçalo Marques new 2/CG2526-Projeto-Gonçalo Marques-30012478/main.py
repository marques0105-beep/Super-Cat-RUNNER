...
from entidades import (Gato, Projetil, BossProjetil, Tronco, Bird, PowerUp, Boss)
from interfaces import (menu, level_select_menu, creditos, level_complete_screen, victory_screen, death_screen)

# Carrega progresso (unlocked levels) ao iniciar
configuracoes.unlocked_levels = load_progress()

def main_game(screen, start_level=1):
    # atualiza GROUND_Y depois de carregar background para que entidades leiam o valor correcto
    clock = pygame.time.Clock()

    # Carrega background e detecta chão (pode usar MANUAL_GROUND_Y)
    ...
    # atualiza configuracoes.GROUND_Y e namespace local
    configuracoes.GROUND_Y = max(0, detected_ground)
    global GROUND_Y
    GROUND_Y = configuracoes.GROUND_Y

    # cria superfície do chão
    ground_height = 56
    ground_surface = create_ground_surface(bg, GROUND_Y, height=ground_height)

    # instancia jogador e ajusta posição
    gato = Gato()
    ...
    while True:
        ...
        if paused:
            ...
            btn_x = WIDTH // 2 - btn_w // 2
            btn_y = HEIGHT // 2 + 50
            draw_button(screen, (btn_x, btn_y, btn_w, btn_h), "Voltar ao Menu", PRIMARY, PRIMARY_HOVER, mouse)
        ...
        if not math.isinf(level_distance) and score >= level_distance:
            # desbloqueia proximo nivel se aplicavel e grava progresso
            if level + 1 > configuracoes.unlocked_levels:
                configuracoes.unlocked_levels = level + 1
                save_progress(configuracoes.unlocked_levels)
            save_highscore(max(score, highscore))
        ...
        if boss_active and boss:
            ...
            if boss.should_shoot_now():
                on_ground = (boss.rect.bottom >= configuracoes.GROUND_Y - 2)
            ...
            if boss.hp <= 0:
                configuracoes.unlocked_levels = max(configuracoes.unlocked_levels, 5)
                save_progress(configuracoes.unlocked_levels)
        ...
        for bullet in bullets[:]:
            ...
            for b_b in boss_bullets[:]:
                if bullet.rect.colliderect(b_b.rect):
                    ...
                    if b_b in boss_bullets: boss_bullets.remove(b_b)
                    hit_bullet = True
                    break
            if hit_bullet: continue
            ...
            for obstacle in obstacles[:]:
                if bullet.rect.colliderect(obstacle.rect):
                    if bullet in bullets: bullets.remove(bullet)
                    if obstacle in obstacles: obstacles.remove(obstacle)
                    hit = True
                    break
            if hit: continue
        ...
        # Spawn e movimento de obstacles se não boss_active
        if not boss_active:
            ...
            if obstacle_spawn_timer <= 0:
                ...
                if len(obstacles) < max_obstacles and rightmost <= WIDTH - min_spacing:
                    ...
                    if level >= 3: bird_prob = min(0.45, bird_prob + 0.06)
        ...
        if powerup_spawn_timer <= 0 and len(powerups) == 0:
            chance = 1 if math.isinf(level_distance) else 3
        ...
        screen.fill(WHITE)
        screen.blit(bg, (bg_x, 0)); screen.blit(bg, (bg_x + WIDTH, 0))
        screen.blit(ground_surface, (bg_x, configuracoes.GROUND_Y)); screen.blit(ground_surface, (bg_x + WIDTH, configuracoes.GROUND_Y))
        gato.draw(screen)
        for b in bullets: b.draw(screen)
        for b_b in boss_bullets: b_b.draw(screen)
        for obstacle in obstacles: obstacle.draw(screen)
        for p in powerups: p.draw(screen)
        if boss_active and boss: boss.draw(screen)
        score += 1
        ...
        hud_x = WIDTH - 260
        screen.blit(score_text, (hud_x, 20)); screen.blit(level_text, (hud_x, 55)); screen.blit(high_text, (hud_x, 90))
        if gato.can_shoot:
